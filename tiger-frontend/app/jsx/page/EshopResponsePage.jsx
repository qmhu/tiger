...
var EshopResponsePage = React.createClass({
	...
	componentDidMount: function(){
		...
		$(".select2").select2();
		var domain_select = $("#domain_select");
		var path_select = $("#path_select");
		...
		$('#landscape').on('ifChanged', function(event){		
			var select_landscape;
			if ($('input[type="radio"].minimal')[0].checked){
				select_landscape = "us";
			}
			
			if ($('input[type="radio"].minimal')[1].checked){
				select_landscape = "cn";
			}
			
			if ($('input[type="radio"].minimal')[2].checked){
				select_landscape = "eu";
			}
			...
			if (select_landscape == window.landscape){
				return;
			}
			
			window.landscape = select_landscape;
			
			$.ajax({
				url: "/api/report/domains" + "?landscape=" + window.landscape
				...
				$("#domain_select").select2("val", "All");
				$("#path_select").select2("val", "");
			});
		...
		domain_select.on("select2:select", function (e) { 
			path_select.empty();
			$.ajax({
				url: "/api/report/contentPath?domain=" + domain_select.val() + "&landscape=" + landscape + "&limit=2000"
			}).then(function(response) {
				response.push("All");
				path_select.select2({
				  data: response
				})
				path_select.select2("val", "All");
			});
		});
		
		$('#search').click(function(){
			var domain = domain_select.val();
			var content_path = path_select.val();
			...
			var chartUrl = "/api/report/responseTime";
			
			var landscape = window.landscape;
			...
			if (aggregate == "Day"){
				chartUrl = chartUrl + "&aggregate=day";
			}else{
				chartUrl = chartUrl + "&aggregate=raw";
			}
			...
			if (content_path && content_path != "All"){
				chartUrl = chartUrl + "&contentPath=" + content_path;
			}
			  ...
			  <div className="row">
				...
				<div className="col-md-6">
				  ...
				  <div className="form-group">
					...
					<select id="aggregate_select" className="form-control select2" defaultValue="Day" style={{width: '100%'}}>
						<option>Day</option>
						<option>Raw</option>
					</select>
				  </div>
...
export default EshopResponsePage